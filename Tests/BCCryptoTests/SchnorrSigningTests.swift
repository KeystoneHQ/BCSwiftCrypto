...
final class SchnorrSigningTests: XCTestCase {
    ...
    func testSchnorrSign() {
        ...
        let signature = schnorrSign(ecdsaPrivateKey: privateKey, message: message, tag: tag, rng: &rng)
        ...
        XCTAssertTrue(schnorrVerify(schnorrPublicKey: schnorrPublicKey, signature: signature, message: message, tag: tag))
    }
}
