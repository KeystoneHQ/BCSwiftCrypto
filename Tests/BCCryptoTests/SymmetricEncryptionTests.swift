...
fileprivate let encrypted = try! Crypto.aeadChaCha20Poly1305Encrypt(plaintext: plaintext, key: key, nonce: nonce, aad: aad)
...
class SymmetricEncryptionTests: XCTestCase {
    func testRFCTestVector() throws {
        ...
        let decryptedPlaintext = try Crypto.aeadChaCha20Poly1305Decrypt(ciphertext: ciphertext, key: key, nonce: nonce, aad: aad, auth: encrypted.auth)
    ...
    func testRandomKeyAndNonce() throws {
        ...
        let (ciphertext, auth) = try Crypto.aeadChaCha20Poly1305Encrypt(plaintext: plaintext, key: key, nonce: nonce, aad: aad)
        let decryptedPlaintext = try Crypto.aeadChaCha20Poly1305Decrypt(ciphertext: ciphertext, key: key, nonce: nonce, aad: aad, auth: auth)
        XCTAssertEqual(plaintext, decryptedPlaintext)
    }
    ...
    func testEmptyData() throws {
        let key = secureRandomData(32)
        let nonce = secureRandomData(12)
        let (ciphertext, auth) = try Crypto.aeadChaCha20Poly1305Encrypt(plaintext: Data(), key: key, nonce: nonce)
        let decryptedPlaintext = try Crypto.aeadChaCha20Poly1305Decrypt(ciphertext: ciphertext, key: key, nonce: nonce, auth: auth)
