...
fileprivate let encrypted = try! Crypto.encryptAEADChaCha20Poly1305(plaintext: plaintext, key: key, nonce: nonce, aad: aad)
...
class SymmetricEncryptionTests: XCTestCase {
    func testRFCTestVector() throws {
        ...
        let decryptedPlaintext = try Crypto.decryptAEADChaCha20Poly1305(ciphertext: ciphertext, key: key, nonce: nonce, aad: aad, auth: encrypted.auth)
