...
final class HashTests: XCTestCase {
    func testSHA() {
        ...
        let digest512 = sha512(input.utf8Data)
    ...
    func testPBKDF2HMACSHA256() {
        let key = pbkdf2HMACSHA256(pass: "password".utf8Data, salt: "salt".utf8Data, iterations: 1, keyLen: 32)
    ...
    func testHKDFHMACSHA256() {
        ...
        let key = hkdfHMACSHA256(keyMaterial: message, salt: salt, keyLen: 32)
    ...
    func testCRC32() {
        ...
        XCTAssertEqual(crc32data(input), ‡"ebe6c6e6")
        XCTAssertEqual(crc32data(input, littleEndian: true), ‡"e6c6e6eb")
    }
